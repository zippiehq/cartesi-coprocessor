...
STRATEGY_MANAGER=$(jq -r .strategyManager < /cartesi-lambada-coprocessor/contracts/script/input/deployment_parameters_devnet.json)
STRATEGY_ADDRESS=$(jq -r .addresses.erc20MockStrategy < /cartesi-lambada-coprocessor/contracts/script/output/coprocessor_deployment_output_devnet.json )
